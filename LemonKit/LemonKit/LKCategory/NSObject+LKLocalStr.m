...
- (NSString *)getLKLocalString: (NSString *)stringKey{
    if (!_lk_stringsFileMap) {
        ...
        if (keyMapPath == nil) {
            NSLog(@"Sorry, the ‘LKStringsFileMapping.plist’ not found in main bundle!");
            return nil;
        }
    ...
    if (stringKeyComponents.count < 2) {// 按照下划线拆分后，得到的数组小于2，说明格式不正确
        NSLog(@"Sorry, your incoming key does not conform to the specification.");
        return nil;
    }
    ...
    if (!fileName) {// mapping plist文件里面没有配置这个key对应的strings文件
        NSLog(@"Sorry, your mapping plist file does not contain the file key!");
        return nil;
    }
